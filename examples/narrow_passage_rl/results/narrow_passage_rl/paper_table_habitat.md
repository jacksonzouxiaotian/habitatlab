...
# under perturbation and reports clearance-aware strict success.
# Full stress summary: paper_table_habitat_stress.md
...
The full module-sensitivity table under Habitat perturbations is generated by
`examples/narrow_passage_rl/eval_habitat_stress_validation.py` as
`paper_table_habitat_stress.md` and `paper_table_habitat_stress.tex`.  It covers
start yaw perturbation (0°, 30°, 60°), lateral start offset (0.0 m, 0.10 m,
0.20 m), depth-sector dropout (0%, 10%, 30%), feature Gaussian noise
(σ=0.0, 0.02, 0.05), and the extreme-narrow subset (`body_margin < 0.05 m`).
This stress validation is the evidence for module sensitivity under
perturbation; clearance-aware metrics are diagnostic indicators, and the nominal
HM3D anchor table above is not intended to separate all FSM modules.

### Current stress slice: +60° start-heading perturb, 24 extreme-narrow episodes
...
- FSM ablation (full / no_recovery / no_alignment): all 100% on unperturbed val
  because mined anchors are mostly well aligned.  The +60° extreme-narrow stress
  slice isolates heading alignment as the critical module, while the full stress
  table (`paper_table_habitat_stress.md`) reports dropout, noise, lateral offset,
  and clearance-aware strict success.
