...
# Geometry-FSM rows are nominal anchor-validation results, not a broad
# robustness claim.  Failure memory is not expected to improve one-shot
# passable-anchor SR; its evidence comes from repeated false-feasible /
# cross-episode experiments.  Stress validation isolates module sensitivity
# under perturbation. Clearance-aware strict success is a diagnostic body-margin
# proxy and is reported separately from nominal stress metrics.
# Split stress tables:
...
|:---|:---:|:---:|:---:|:---:|:---|
| Legacy PPO-SB3 (excluded from formal baselines) | 0.0% | 0.0% | 0.0% | 0.0% | Historical synthetic v1 obs[10] mismatch vs. Habitat; provenance only |
...
| **Geometry-FSM (ours)** | **100%** | **100%** | **100%** | **100%** | 151/151 nominal anchor validation |
| FSM w/o recovery | **100%** | **100%** | **100%** | **100%** | Nominal anchors do not trigger recovery differences |
| FSM w/o alignment | **100%** | **100%** | **100%** | **100%** | Nominal anchors are mostly well aligned |

## Habitat Stress Validation

The full module-sensitivity protocol is generated by
`examples/narrow_passage_rl/eval_habitat_stress_validation.py`. It covers start
yaw perturbation (0°, 30°, 60°), lateral start offset (0.0 m, 0.10 m, 0.20 m),
depth-sector dropout (0%, 10%, 30%), feature Gaussian noise (σ=0.0, 0.02,
0.05), and the extreme-narrow subset (`body_margin < 0.05 m`).

Paper-facing stress outputs are split to avoid over-interpreting clearance
diagnostics:

- `tables/paper_table_habitat_stress_nominal.md`: success, collision, timeout,
  and average steps.
- `tables/paper_table_habitat_clearance_diagnostic.md`: strict success,
  near-collision, min-clearance, and success-but-unsafe as depth-derived
  body-margin diagnostics.
- `tables/paper_table_habitat_stress_key_slices.md`: compact slices for
  nominal, yaw60, extreme_yaw60, and lat020 discussion.

Current key-slice result: on `extreme_yaw60`, full FSM reaches 100.0% nominal
success, while removing heading alignment drops to 79.2% with 12.5% timeout.
This is module sensitivity, not a complete failure claim.
...
- min_clearance<0 is a depth-sensor artifact at valid navmesh positions near walls,
  not calibrated wall penetration.
- FSM ablation (full / no_recovery / no_alignment): all 100% on unperturbed val
  because mined anchors are mostly well aligned.  The +60° and extreme-narrow
  stress slices expose heading-alignment sensitivity; see
  `tables/paper_table_habitat_stress_key_slices.md`. Clearance-aware strict
  success is split into `tables/paper_table_habitat_clearance_diagnostic.md` and
  should be treated as a diagnostic body-margin proxy.
