...
| Habitat PointNav / ObjectNav pipeline | 已完成诊断性运行 | PointNav 保留 50-episode aggregate 与视频；ObjectNav 保留 20-episode `val_mini` aggregate，正式 held-out multi-seed 评测待补 |
...
| 已完成（平台验证） | Habitat PointNav / ObjectNav | PointNav：50 episodes，SR `94.0%`、SPL `0.792`；ObjectNav：20 episodes，SR `15.0%`、SPL `0.113`、SoftSPL `0.640` |
...
PointNav 的 50 个视频包含 47 个成功和 3 个失败 episode，并与 TensorBoard aggregate 一致。ObjectNav 当前只保留单次 aggregate。上述两行证明标准 Habitat task pipeline 已在本地执行，但不构成跨任务排行榜，也不能与 `NarrowPassageNav-v0` 的 same-split canonical comparison 混排。

结果来源：

- PointNav：`tb/events.out.tfevents.1779936036.xiaotian-HP-Z2-Tower-G9-Workstation-Desktop-PC.289068.0` 与 `video_dir/episode=*_1-ckpt=0-*.mp4`。
- ObjectNav：`tb/events.out.tfevents.1780897743.xiaotian-HP-Z2-Tower-G9-Workstation-Desktop-PC.2131990.0`。
- 汇报用独立结果表：`examples/narrow_passage_rl/results/narrow_passage_rl/tables/report_table_habitat_general_navigation.md`。
...
| Method                     |        SR@75 ↑ |        SR@85 ↑ |        SR@95 ↑ | Avg. Collision ↓ | Avg. Oscillation ↓ | Successful Avg. Time ↓ |
| -------------------------- | -------------: | -------------: | -------------: | ---------------: | -----------------: | ---------------------: |
| Nav2 + DWB                 |     2/10 (20%) |     4/10 (40%) |     6/10 (60%) |                7 |    **9.60 ± 6.47** |        72.52 ± 17.23 s |
| Nav2 + TEB                 |     3/10 (30%) |     5/10 (50%) |     7/10 (70%) |                5 |       35.27 ± 9.35 |         74.22 ± 8.93 s |
| Nav2 + MPPI                |     4/10 (40%) |     6/10 (60%) |     7/10 (70%) |                3 |       26.60 ± 2.41 |        62.41 ± 12.85 s |
| Nav2 + RPP                 |     2/10 (20%) |     4/10 (40%) |     6/10 (60%) |                4 |       28.10 ± 6.42 |        94.52 ± 17.37 s |
| DEGNAV                     | **5/10 (50%)** | **7/10 (70%)** | **8/10 (80%)** |            **2** |       22.90 ± 5.61 |    **52.85 ± 16.88 s** |
...
| Method                     |        SR@75 ↑ |        SR@85 ↑ |        SR@95 ↑ | Avg. Collision ↓ | Avg. Oscillation ↓ | Successful Avg. Time ↓ |
| -------------------------- | -------------: | -------------: | -------------: | ---------------: | -----------------: | ---------------------: |
| DEGNAV w/o Memory          |     4/10 (40%) |     5/10 (50%) |     7/10 (70%) |                4 |       10.80 ± 3.42 |        68.43 ± 20.71 s |
| DEGNAV w/o Posture Control |     4/10 (40%) |     5/10 (50%) |     6/10 (60%) |                4 |       12.00 ± 4.33 |        73.97 ± 13.75 s |
| DEGNAV                     | **5/10 (50%)** | **7/10 (70%)** | **8/10 (80%)** |            **2** |       22.90 ± 5.61 |    **52.85 ± 16.88 s** |
...
机器人在狭窄通道入口出现连续左右振荡，多次调整入口角度，但没有形成稳定的 forward commitment，最终耗尽时间预算。从 rosbag 中补充：
...
核验 Lite3 多次重复统计和完整闭环结果。上传 rosbag、外部视频、episode CSV、传感器标定和 baseline 配置。正式材料写为 experiment design 或 pilot protocol，不能写“广泛实机验证”或“完整 sim-to-real 成功”。
...
| PointNav / ObjectNav pipeline checks | 标准任务接口已跑通；现有单 checkpoint / `val_mini` 结果不构成正式泛化证据 |
...
8. PointNav `94.0%` 与 ObjectNav `15.0%` 来自不同任务和协议，不能相互排名，也不能直接支撑 HM3D held-out 泛化。
...
7. PointNav / ObjectNav pipeline diagnostic。
8. DEGNAV-RL diagnostic。
9. Isaac locomotion 多 seed 结果，待生成。
10. Lite3 实机重复实验结果，待生成。
...
> 当前证据表明，在接近几何可行性边界的窄通道中，显式几何对齐与模式决策比 reactive rule 更稳定；failure memory 的主要价值是减少重复不可行 commitment，Lite3 闭环仍处于系统集成与正式评测阶段。

## 15.2 方法页

依次展示 VLN、Geometry / Belief、Failure Memory、Mode Decision 和 RL Locomotion。
...
正式实验使用 3 个随机 seeds，并固定场景、指令、初始状态与评测预算。Habitat 与 Isaac Sim 的结果应分表报告；真实 Lite3 结果应给出每个条件的试验次数、成功数、失败类型和置信区间。
...
|---|---|---|
| Habitat-Lab navigation | 已完成 | Narrow Passage 有 episode CSV、same-split 与 stress 表；PointNav/ObjectNav 有诊断性 pipeline artifacts |
...
1. 跑通标准 PointNav pipeline：Habitat test-scenes val 的 50 episodes 上，SR `94.0%`、SPL `0.792`、最终距离 `0.114 m`。
2. 跑通 ObjectNav DD-PPO pipeline：HM3D ObjectNav v2 `val_mini` 的 20-episode smoke evaluation 上，SR `15.0%`、SPL `0.113`、SoftSPL `0.640`、最终距离 `1.074 m`。
3. 建立 `NarrowPassageNav-v0` 与 19-D geometry observation。
4. 建立 procedural v2、HM3D nominal anchor 和 stress validation。
5. DEGNAV-Rule 在 procedural v2 上达到 `70.3±1.4%` Overall SR，Reactive rule 为 `25.4±1.2%`。
6. 在 HM3D same-split nominal anchors 上完成 157-episode 对照；100% 结果仅解释为当前 mining protocol 下的 nominal validation。
...
9. **General-navigation provenance gap**：PointNav 缺 HM3D 多 seed per-episode CSV；ObjectNav 仅有 `val_mini` single-run aggregate，尚不能形成 held-out 泛化结论。
...
1. 将已有 PointNav/ObjectNav pipeline checks 升级为带 per-episode CSV、固定 seeds 和 held-out split 的正式 baseline，并补充 VLN seen/unseen 评测。
...
> 本项目已跑通 Habitat PointNav 与 ObjectNav 标准任务 pipeline，并建立 Habitat-Lab 窄通道导航、显式几何决策和 Failure Memory 的可复现实验链路，同时形成 Isaac Sim 四足强化学习与 DeepRobotics Lite3 实机系统的实验框架。当前 PointNav/ObjectNav 数据用于平台与接口验证，不作为 held-out 泛化结论。窄通道可核验结果表明，显式入口对齐与 passage-centric geometry 在接近可行性边界的场景中优于 reactive rule，geometry-guided memory 在合成重复失败协议中能够减少不可行通道的重复尝试，同时避免 generic memory 的过度拒绝。用户已报告完成 Isaac Sim 训练和 Lite3 验证，但论文级 checkpoint、训练曲线、rosbag 和多次重复统计仍需归档。下一阶段工作的重点不是继续扩大未经校验的功能范围，而是完成 VLN 接口、统一仿真与实机 protocol，并把已有系统运行转化为可复现、可审查的论文证据。
